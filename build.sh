...
INSTALLER_BASENAME="gitlab-installer"
...
Notes:
  GitLab CE upstream container images are heavy. Build on a machine with enough disk space.
  This package currently ships the amd64 GitLab CE image definition by default.
USAGE
}
...
check_requirements() {
  ...
  [[ -d "${ROOT_DIR}/manifests" ]] || die "manifests directory is missing"
  grep -q '^__PAYLOAD_BELOW__$' "${INSTALLER_TEMPLATE}" || die "install.sh is missing __PAYLOAD_BELOW__ marker"
  jq empty "${IMAGE_JSON}" >/dev/null
}

prepare_directories() {
  rm -rf "${TEMP_DIR}"
  mkdir -p "${PAYLOAD_DIR}/manifests" "${PAYLOAD_DIR}/images" "${DIST_DIR}"
}
...
prepare_images() {
  ...
  while IFS= read -r item; do
    ...
    local pull dockerfile default_target_ref tar_name load_ref item_platform name
    ...
    [[ -n "${default_target_ref}" && "${default_target_ref}" != "null" ]] || die "Missing tag in image.json item: ${item}"
    [[ -n "${tar_name}" && "${tar_name}" != "null" ]] || die "Missing tar in image.json item: ${item}"
    [[ -n "${pull}" || -n "${dockerfile}" ]] || die "Either pull or dockerfile is required for ${name:-${tar_name}}"
    load_ref="$(build_local_load_ref "${default_target_ref}")"

    if [[ -n "${dockerfile}" ]]; then
      [[ -f "${ROOT_DIR}/${dockerfile}" ]] || die "Dockerfile not found: ${dockerfile}"
      log "Building ${name:-${default_target_ref}} for ${item_platform}"
      docker buildx build --load --platform "${item_platform}" -t "${load_ref}" -f "${ROOT_DIR}/${dockerfile}" "${ROOT_DIR}"
    else
...
package_payload() {
  ...
  log "Copying manifest payload"
  cp -R "${ROOT_DIR}/manifests/." "${PAYLOAD_DIR}/manifests/"
...
main() {
  ...
  if [[ "${BUILD_ALL_ARCH}" == "true" ]]; then
    if arch_has_images "amd64"; then
      build_one "amd64" "linux/amd64"
    else
      warn "Skipping amd64 because no image definitions exist"
    fi
    if arch_has_images "arm64"; then
      build_one "arm64" "linux/arm64"
    else
      warn "Skipping arm64 because no image definitions exist"
    fi
  else
